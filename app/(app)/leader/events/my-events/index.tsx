...
const MyEventsScreen = () => {
    ...
    const fetchMyEvents = async () => {
        try {
            ...
            if (response?.data?.data) {
                // Lọc ra các event có dữ liệu eventId hợp lệ
                const validEvents = response.data.data.filter(
                    (event: any) => event && event.eventId && typeof event.eventId === 'object'
                );
    ...
    const handleUnregister = async (eventId: string) => {
        try {
            if (!eventId) {
                ...
                Alert.alert('Lỗi', 'Không thể hủy đăng ký vì thiếu thông tin sự kiện');
                return;
            }
    ...
    const renderEventItem = ({ item }: { item: EventRegistration }) => {
        ...
        if (!item.eventId) {
            console.warn('Dữ liệu sự kiện không hợp lệ:', item);
            return (
                <View className="bg-white p-4 mb-4 rounded-lg shadow-sm">
                    <Text className="text-red-500">Dữ liệu sự kiện không hợp lệ</Text>
                </View>
            );
        }
            ...
            <View className="bg-white p-4 mb-4 rounded-lg shadow-sm">
                <View className="flex-row justify-between items-start">
                    <View className="flex-1 mr-4">
                        <Text className="font-bold text-lg text-gray-900">{item.eventId.name || 'Không có tên'}</Text>

                        <View className="mt-2">
                            <View className="flex-row items-center mb-1">
                                <Ionicons name="time-outline" size={16} color="#666" />
                                <Text className="text-gray-600 ml-2">
                                    {item.eventId.startedAt ? formatEventTime(item.eventId.startedAt, item.eventId.endedAt) : 'Không có thời gian'}
                                </Text>
                            </View>

                            <View className="flex-row items-center mb-1">
                                <Ionicons name="location-outline" size={16} color="#666" />
                                <Text className="text-gray-600 ml-2">{item.eventId.location || 'Không có địa điểm'}</Text>
                            </View>

                            <View className="flex-row items-center mb-1">
                                <Ionicons name="people-outline" size={16} color="#666" />
                                <Text className="text-gray-600 ml-2">{item.eventId.scope || 'Không có phạm vi'}</Text>
                            </View>
                    ...
                    <TouchableOpacity
                        ...
                        onPress={() => {
                            if (!item.eventId || !item.eventId._id) {
                                Alert.alert("Lỗi", "Không thể hủy đăng ký do thiếu thông tin sự kiện");
                                return;
                            }
